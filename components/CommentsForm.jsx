...
const CommentsForm = ({ slug }) => {
    ...
    //we don't need these values in state so we simply read from input and then send to graphcms
    const commentEl= useRef();
    const nameEl= useRef();
    const emailEl= useRef();
    const storeDataEl= useRef();
    
    //Fetching data from local storage if we come back to comment
    useEffect(()=>{
    ...
    return (
        <div className="bg-white shadow-lg rounded-lg p-8 pb-12 mb-8">
            ...
            <div className="grid grid-cols-1 mt-8 gap-4 mb-4">
                <div>
                    <input
                        ref={storeDataEl}
                        type="checkbox"
                        ...
                        value="true"
                    />
                    <label className="text-gray-500 cursor-500 px-2" htmlFor="storeData">Save my e-mail and name for the next time I comment</label>
                </div>
            </div>
